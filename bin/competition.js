		 ...
		 train_db.open(function(err, train_db) {
		 	...
		 	async.waterfall([   //time to do things one at a time
			 	...
			 	/*	taglist.find().toArray(function(err, items) {   //BUILD TAG DICTIONARY ARRAY
						...
						the_taglist = require('../../Data/top_tag_FILTERED_laptop_12_16_1pm.js');  //EXTERNAL TAG LIST SORTED AND CUT BY FREQUENCY IN TRAIN DATA
				...
				//	});
			 	...
			 	function(taglist, callback) {   //PROCESS TEST SET IN CSV STREAM
			 		...
			 		var stream = fs.createWriteStream('../../Data/submission_laptop_12_16_1pm_topoff2.csv', {flags: 'w'}); //create write stream and point towards output file
			 	//	var header = '"Id","Tags"' + '\n';
			 	//	stream.write(header);                                      //manually create header
			 			 	         		
        			var data = {};
		        	var __dirname = '../../Data',
						__inname = '/Test_4_2.csv',
						...
						.on('record', function(row,index){
							...
							async.waterfall([   //time to do things one at a time
									...
									for(u in taglist){    //FIND CANDIDATE TAGS IN TEST ROW
										...
										if(tagstop(taglist[u]) === true){
											...
											if(title_candidates.indexOf(taglist[u]) === (-1) && body_candidates.indexOf(taglist[u]) === (-1)){   //TEMPORARY STOP WORD FILTERS, REMOVE FROM DICTIONARY DB COLLECTION INSTEAD BEFORE PRODUCTION
												...
												test_tag = taglist[u].replace("-"," ");

												if(title != undefined && title.indexOf(" " + test_tag + " ") != (-1) ){
													title_candidates.push(taglist[u]); 
												//	console.log("Title candidate: " + taglist[u] + "  in  TITLE: " + row[1]);
												} 

												else if(body != undefined && body.indexOf(" " + test_tag + " ") != (-1)     ){
								...
								//	console.log("BODY TAGS: " + body_candidates);
									...
									the_tags = title_candidates.concat(body_candidates);
									the_tags = the_tags.slice(0,5);
									the_tags = the_tags.join(" ");

									if(Id === "Id"){ Id = '"Id"'; the_tags = 'Tags'; }

									//console.log(Id);
									callback(null, the_tags, Id);

								},

								function(the_tags, Id, callback) {       //WRITE TO OUTPUT FILE
											...
											vectorcollection.update({ Term: a_tag },    //write to workhorse data science dictionary
												{	
													...
													function (err, document){
															...
															else{ console.log("TEST @@@@@@" + a_tag); callback(); }
