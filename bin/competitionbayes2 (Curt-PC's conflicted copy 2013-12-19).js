							...
							async.waterfall([   //time to do things one at a time
								...
								function(callback) {    				//PROCESS TEST DATA
									...
									for(var u = 0; u < select_tags.length; u++){    
										...
										if(select_tags[u].Title_num < 40 && training_tags.indexOf(select_tags[u].Tag) === (-1) ){     //CUTT OFF NUMBER OF DOCS USED FOR TRAINING
										...
										if(select_tags[u].Body_num < 20 && training_tags.indexOf(select_tags[u].Tag) === (-1) ){     //CUTT OFF NUMBER OF DOCS USED FOR TRAINING
			 ...
			 /*		var natural = require('natural'),
				...
				callback(null, title_tag_classifier, body_tag_classifier);
			 	...
			 	function(title_tag_classifier, body_tag_classifier, callback) {   //SAVE CLASSIFIERS
					...
					var stream = fs.createWriteStream('../../Data/test_superbayes_against_train_2d.csv', {flags: 'w'}); //create write stream and point towards output file
			 	//	var header = '"Id","Tags"' + '\n';
			 	//	stream.write(header);                                      //manually create header
			 			 	         		
        			var data = {};
		        	var __dirname = '../../Data',
						__inname = '/Test_2d.csv',
						__outname = '/null_out.csv';
					csv()
						.from.path(__dirname+__inname, { delimiter: ',', escape: '"' })
						.to.stream(fs.createWriteStream(__dirname+__outname))
									...
									//	console.log("TAG: " + classify_title_results[3].label + "SCORE: " + classify_title_results[3].value/title_average_score);	
										...
										for(var z = 0; z < classify_title_results.length; z++){
											...
											if( (classify_title_results[z].value / title_average_score) > title_cutoff){
												...
												found_tags.push(classify_title_results[z].label);	
									...
									if(classify_body_results !== undefined){
										...
										for(var r = 0; r < classify_body_results.length; r++){
											...
											if( (classify_body_results[r].value/body_average_score) > body_cutoff && found_tags.indexOf(classify_body_results[r].label) === (-1)){
												found_tags.push(classify_body_results[r].label);	
